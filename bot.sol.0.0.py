CREATE TABLE IF NOT EXISTS solana_requests (
...
def handle_solana_choice(update: Update, context: CallbackContext):
    ...
    if query.data == "solana_existing":
        ...
        process_solana_request(user_id, "new", None)

def process_solana_request(user_id: int, request_type: str, address: str = None):
    ...
    bot.send_message(chat_id=admin_chat_id, text=f"New Solana setup request from user {user_id} (Type: {request_type}).")
...
def approve_solana(update: Update, context: CallbackContext):
    ...
    bot.send_message(chat_id=user_id, text=f"Your Solana address has been approved: {solana_address}")
